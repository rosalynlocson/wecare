                ...
                <form method="POST" action="{{ route('appointments.update', $appointment) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="patient_id" class="mt-1 block w-full rounded-md border-gray-300">
                            ...
                            @foreach ($patients as $patient)
                                <option value="{{ $patient->id }}" @selected(old('patient_id', $appointment->patient_id) == $patient->id)>{{ $patient->fullName() }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="treatment_type_id" class="mt-1 block w-full rounded-md border-gray-300">
                            ...
                            @foreach ($treatmentTypes as $type)
                                <option value="{{ $type->id }}" @selected(old('treatment_type_id', $appointment->treatment_type_id) == $type->id)>{{ $type->name }} (₱{{ $type->default_price }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="date" name="appointment_date" value="{{ old('appointment_date', $appointment->appointment_date->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300">
                        ...
                        <div>
                            ...
                            <input type="time" name="appointment_time" value="{{ old('appointment_time', $appointment->appointment_time->format('H:i')) }}" class="mt-1 block w-full rounded-md border-gray-300">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="notes" class="mt-1 block w-full rounded-md border-gray-300">{{ old('notes', $appointment->notes) }}</textarea>
                    </div>

                    <div class="mt-6 flex gap-2">
                        <a href="{{ route('appointments.index') }}" class="px-4 py-2 border rounded-md text-sm text-gray-600">Cancel</a>
                        <button type="submit" class="px-4 py-2 bg-gray-900 text-white rounded-md text-sm">Update Appointment</button>
                    </div>
                </form>
            </div>
        </div>
